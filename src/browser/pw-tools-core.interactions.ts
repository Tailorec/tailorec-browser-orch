 ...
 *
 * Strategy escalation (mirrors Skyvern handler.py handle_input_text_action):
 * 1. If current value already matches → skip
 * 2. For type="date" → format-aware fill (ISO, MM/DD/YYYY, etc.)
 * 3. For type="tel" with masked placeholder → digits-only via pressSequentially
 * 4. Try locator.fill()
 * 5. Read back value. If matches → done
 * 6. If mismatch → clear + pressSequentially (char by char with 40ms delay)
 * 7. Read back again. If mismatch → report warning with actual vs requested
 *
 * Skyvern reference:
 *   handler.py lines 540-1140 (handle_input_text_action)
 *   handler.py line 1060: input_sequentially fallback
 *   handler.py line 220-305: check_phone_number_format / check_date_format
 */
export async function fillAndVerifyField(
  ...
  // Step 0.5: Format-aware handling for specific input types
  const placeholder = await locator.getAttribute("placeholder", { timeout: 1500 }).catch(() => "");

  // Date input handling (Skyvern: handler.py check_date_format lines 262-305)
  ...
  // Phone format handling (Skyvern: handler.py check_phone_number_format lines 220-260)
  if (inputType === "tel") {
  ...
  try {
    ...
    // Fallback: click + selectText + keyboard.type (Skyvern: input_sequentially)
    try {
  ...
  if (!result.matched) {
    result.warning = `Value mismatch after fill: requested="${value.slice(0, 50)}" actual="${result.actualValue.slice(0, 50)}"`;
  }
...
export async function takeScreenshotViaPlaywright(opts: {
  ...
  if (opts.ref) {
    ...
    const buffer = await locator.screenshot({
      type,
      ...(type === "jpeg" && typeof opts.quality === "number" ? { quality: opts.quality } : {}),
    });
    return { buffer };
  }
  if (opts.element) {
...
// ─── Blocking Element Detection (Plan 06) ─────────────────────────────
 ...
 *
 * Skyvern reference:
 *   dom.py: find_blocking_element() line 496
 *   page.py: get_blocking_element_id() line 416
 *   domUtils.js: getBlockElementUniqueID() line 640
 */
export async function detectBlockingElementViaPlaywright(opts: {
  ...
  const result = await locator.evaluate((target: Element) => {
    ...
    const closeSelectors = [
      ...
      'button[aria-label*="accept" i]', 'button[class*="close" i]',
      'button[class*="dismiss" i]', '[role="button"][aria-label*="close" i]',
      'a[class*="close" i]', 'button:has(svg)',
    ];
    ...
    for (const selector of closeSelectors) {
      ...
      if (found) {
        ...
        if (foundRect.width > 0 && foundRect.height > 0) { closeButton = found; break; }
      }
    }
    ...
    if (!closeButton) {
      ...
      for (const btn of buttons) {
        if (acceptPatterns.test(btn.textContent || "")) {
          ...
          if (btnRect.width > 0 && btnRect.height > 0) { closeButton = btn; break; }
        }
      }
    }
    ...
    return {
      ...
      blockerRect: { x: containerRect.x, y: containerRect.y, width: containerRect.width, height: containerRect.height },
      dismissStrategy,
...
/**
 * Attempt to dismiss a blocking overlay with escalating strategies.
 */
export async function dismissBlockerViaPlaywright(opts: {
  ...
  for (const strategy of strategies) {
    try {
      ...
      if (!check.isBlocked) return { dismissed: true, strategy };
    } catch {
      // try next strategy
    }
  }
