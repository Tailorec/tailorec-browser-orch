...
async function captureScreenshot(opts) {
    return await (0, cdp_helpers_js_1.withCdpSocket)(opts.wsUrl, async (send) => {
...
async function createTargetViaCdp(opts) {
    const version = await (0, cdp_helpers_js_1.fetchJson)((0, cdp_helpers_js_1.appendCdpPath)(opts.cdpUrl, "/json/version"), 1500);
    const wsUrlRaw = String(version?.webSocketDebuggerUrl ?? "").trim();
    const wsUrl = wsUrlRaw ? normalizeCdpWsUrl(wsUrlRaw, opts.cdpUrl) : "";
    if (!wsUrl) {
        throw new Error("CDP /json/version missing webSocketDebuggerUrl");
    }
    return await (0, cdp_helpers_js_1.withCdpSocket)(wsUrl, async (send) => {
