...
const errors_js_1 = require("../infra/errors.js");
...
const log = (0, subsystem_js_1.createSubsystemLogger)("pw-session");
    ...
    const connectWithRetry = async () => {
        ...
        for (let attempt = 0; attempt < 3; attempt += 1) {
            try {
                ...
                const wsUrl = await (0, chrome_js_1.getChromeWebSocketUrl)(normalized, timeout).catch(() => null);
                const endpoint = wsUrl ?? normalized;
                const headers = (0, cdp_helpers_js_1.getHeadersWithAuth)(endpoint);
                const browser = await playwright_core_1.chromium.connectOverCDP(endpoint, { timeout, headers });
        ...
        const message = lastErr ? (0, errors_js_1.formatErrorMessage)(lastErr) : "CDP connect failed";
...
async function findPageByTargetId(browser, targetId, cdpUrl) {
    ...
    if (cdpUrl) {
        try {
            ...
            const response = await fetch(listUrl, { headers: (0, cdp_helpers_js_1.getHeadersWithAuth)(listUrl) });
            if (response.ok) {
